...
def act_func(make_obs_ph_n, act_space_n, p_index, p_func, scope="trainer", num_units=64, reuse=None):
    tf.random.set_random_seed(2021)
    with tf.variable_scope(scope, reuse=reuse):
        # create distribtuions
        act_pdtype_n = [make_pdtype(act_space) for act_space in act_space_n]
...
class MADDPGAgentTester(AgentTrainer):
    def __init__(self, name, model, obs_shape_n, act_space_n, agent_index, args, local_q_func=False):
        ...
        self.act = act_func(make_obs_ph_n=obs_ph_n, act_space_n=act_space_n, p_index=agent_index, p_func=model, scope=self.name, num_units=args.num_units)
