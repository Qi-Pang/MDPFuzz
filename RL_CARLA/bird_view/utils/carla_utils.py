...
class CarlaWrapper(object):
    ...
    def spawn_vehicles(self):
        if self.replay:
            vehicle_info = self.replayer.get_vehicle_info()
            # print('vehicle1: ', len(vehicle_info))
            current_x = self.replayer.current_pose.location.x
            current_y = self.replayer.current_pose.location.y
            ...
            legitimate_vehicle_info = vehicle_info
            ...
            self.replayer.current_vehicle_info = legitimate_vehicle_info
            ...
            #     temp_x = spawn_points[i].location.x
            #     temp_y = spawn_points[i].location.y
            #     min_dis = np.inf
            #     for j in range(len(spawn_points)):
            #         if i == j:
            #             continue
            #         delta_x = temp_x - spawn_points[j].location.x
            #         delta_y = temp_y - spawn_points[j].location.y
            #         temp_dis = np.linalg.norm([delta_x, delta_y])
            #         if temp_dis < min_dis:
            #             min_dis = temp_dis
            # print('min distance', min_dis)
            # print('spawn points', spawn_points[0], len(spawn_points))
    ...
    def init(self, start=0, target=1, weather="random", n_vehicles=0, n_pedestrians=0):
        ...
        if self.replay:
            self._target_pose = self._map.get_spawn_points()[self.replayer.current_envsetting[1]]
            self.set_weather(self.replayer.current_envsetting[2])
        elif self.first_run:
    ...
    def spawn_player(self):
        ...
        if self.replay:
            self._start_pose = self.replayer.get_pose()
        elif self.first_run:
            pass
        ...
        print('player id: ', self._player.id)
        # Mutation here:
