...
class ActorCriticPolicy(BasePolicy):
    ...
    def _get_latent(self, obs: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        # FIXME: here is the activation
        latent_pi, latent_vf = self.mlp_extractor(features)
        # ffff1 = latent_pi.clone().cpu().detach().numpy()
        # ffff2 = latent_vf.clone().cpu().detach().numpy()
        # ffff = np.hstack((ffff1, ffff2))
        # self.storage.append(ffff[0])
        # if len(self.storage) > 1000 * self.index_flag:
        #     with open('/export/d1/qpangaa/github/MDP-Testing/RL/rl-baselines3-zoo/results/tsne_crash.pkl', 'wb') as handle:
