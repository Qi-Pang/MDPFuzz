...
class ImagePolicyModelSS(common.ResnetBase):
    ...
    def forward(self, image, velocity, command):
        ...
        h = self.conv(image)
        temp = torch.sign(F.relu(h.clone()))
        temp = temp[0]
        temp = torch.mean(temp, axis = 0).flatten()
        b, c, kh, kw = h.size()
        ...
        if self.all_branch:
            return location_pred, location_preds, temp
        
        return location_pred, temp
    ...
    def run_step(self, observations, teaching=False):
        ...
        with torch.no_grad():
            ...
            if self.model.all_branch:
                model_pred, _, temp = self.model(_rgb, _speed, _command)
            else:
                model_pred, temp = self.model(_rgb, _speed, _command)

        model_pred = model_pred.squeeze().detach().cpu().numpy()
        entropy = np.var(model_pred, axis=0).sum()
